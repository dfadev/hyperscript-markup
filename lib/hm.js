		...
		visitor: {
			BinaryExpression: function BinaryExpression(path) {
				...
				if (t.isIdentifier(path.node.left)) {
					...
					if (nm[0] != "$") return; // || nm[1] != "$") return;
					if (nm.length > 1) functionPrefix = t.identifier(nm.substring(1));
				} else if (t.isMemberExpression(path.node.left)) {
					var clean = (0, _babelGenerator2.default)(path.node.left, { comments: false });

					var _nm = clean.code;
					if (_nm[0] != "$") return; // || nm[1] != "$") return;
					if (_nm.length > 1) functionPrefix = babylon.parseExpression(_nm.substring(1));
				}
